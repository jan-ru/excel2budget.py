...
def generate_archimate_diagram(
    ...
    intermediaries = (
        ", ".join(s.name for s in context.intermediarySystems)
        if context.intermediarySystems
        else ""
    )

    content = (
        archimate_template.templateContent if archimate_template.templateContent else ""
    )
...
def _generate_archimate_svg(source, app, target, intermediaries):
    ...
    svg = (
        ...
        f'width="{width}" height="140">' + "".join(boxes) + "</svg>"
    )
    return svg
...
def _generate_bpmn_svg(steps):
    ...
    # Start event
    elements.append(
        f'<circle cx="{x + 15}" cy="70" r="15" fill="#c8e6c9" stroke="#388e3c"/>'
    )
    x += 50
    ...
    svg = (
        f'<svg xmlns="http://www.w3.org/2000/svg" '
        f'width="{width}" height="140">' + "".join(elements) + "</svg>"
    )
    return svg
