...
- [ ] 1. Create backend domain model types in `backend/app/core/domain.py`
  - [ ] 1.1 Define primitive types and enums (`AccountCode`, `EntityCode`, `Period` NewTypes, `LineType`, `AccountType`, `DebitCredit` StrEnums)
  ...
  - [ ] 1.2 Define dimension models (`Account`, `Entity`, `Period`)
  ...
  - [ ] 1.3 Define core line types (`FinancialLine`, `BudgetLine`, `ActualLine`, `ForecastLine`)
  ...
  - [ ] 1.4 Define statement lines and `FinancialDocument`
  ...
  - [ ] 1.5 Write property test: Immutability enforcement (Property 1)
  ...
  - [ ] 1.6 Write property test: Specialised line type Literal constraint (Property 2)
  ...
  - [ ] 1.7 Write property test: Tuple collections (Property 3)
  ...
  - [ ] 1.8 Write property test: model_copy produces new instance (Property 4)
  ...
  - [ ] 1.9 Write property test: JSON serialization round-trip (Property 7)
...
- [ ] 2. Create Hypothesis test strategies in `backend/tests/strategies.py`
...
- [ ] 3. Checkpoint — Verify domain model types
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Implement pure function layer in `backend/app/core/functions.py`
  - [ ] 4.1 Implement `filter_entity` and `filter_period`
  ...
  - [ ] 4.2 Write property tests for filter functions (Properties 9, 10)
  ...
  - [ ] 4.3 Implement `compute_variance`
  ...
  - [ ] 4.4 Write property test for compute_variance (Property 11)
  ...
  - [ ] 4.7 Implement `to_polars` and `from_polars`
    - `to_polars(doc)` converts `FinancialDocument` lines to a Polars DataFrame with columns matching `FinancialLine` field names
    - `from_polars(df)` reconstructs `FinancialDocument` from DataFrame rows (accounts, entities, meta default to empty)
    - _Requirements: 6.5, 6.6_

  - [ ] 4.8 Write property test for Polars round-trip (Property 8)
    - **Property 8: Polars conversion round-trip**
    - Create `backend/tests/test_property_functions_polars.py`
    - Verify `from_polars(to_polars(doc)).lines` is equivalent to `doc.lines`
    - **Validates: Requirements 6.5, 6.6, 6.7**

- [ ] 5. Checkpoint — Verify pure function layer
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Implement adapter layer in `backend/app/core/adapters.py`
  - [ ] 6.1 Implement `tabular_to_financial_document`
  ...
  - [ ] 6.2 Implement `financial_document_to_tabular`
  ...
  - [ ] 6.3 Write property test for adapter round-trip (Property 17)
    - **Property 17: Adapter TabularData round-trip preserves data values**
    - Create `backend/tests/test_property_adapters.py`
    - **Validates: Requirements 11.3, 11.4, 11.5**

  - [ ] 6.4 Write property tests for writer behaviors (Properties 13–16)
...
- [ ] 7. Checkpoint — Verify adapter layer
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 8. Create frontend Zod schemas in `frontend/src/types/domain.ts`
  - [ ] 8.1 Define enum and primitive Zod schemas
  ...
  - [ ] 8.2 Define dimension and line type Zod schemas
  ...
  - [ ] 8.3 Define `FinancialDocumentSchema` and export inferred types
  ...
  - [ ] 8.4 Write property test: Zod accepts backend JSON (Property 5)
  ...
  - [ ] 8.5 Write property test: Zod error messages (Property 6)
...
- [ ] 9. Checkpoint — Verify frontend Zod schemas
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Refactor frontend pipeline to use `FinancialDocument`
  - [ ] 10.1 Update Excel importer to produce `FinancialDocument`
  ...
  - [ ] 10.2 Update pipeline orchestrator to pass `FinancialDocument` between stages
  ...
  - [ ] 10.3 Update CSV and Excel exporters to consume `FinancialDocument`
  ...
  - [ ] 10.4 Remove `TabularData` references from frontend pipeline code
  ...
  - [ ] 10.5 Write unit tests for frontend pipeline refactoring
